...
for (const { label, editorResult, expectedOutput, expectedStatus } of [
	{
		label: "completed with text",
		editorResult: { output: "fixture applied" },
		expectedOutput: "fixture applied",
		expectedStatus: "completed",
	},
	{
		label: "completed without text",
		editorResult: undefined,
		expectedOutput: "",
		expectedStatus: "completed",
	},
	{
		label: "failed with text",
		editorResult: { status: "failed", output: "fixture failed" },
		expectedOutput: "fixture failed",
		expectedStatus: "failed",
	},
	{
		label: "failed without text",
		editorResult: { status: "failed" },
		expectedOutput: "",
		expectedStatus: "failed",
	},
])
	test(
		`SDK apply_patch continuation: ${label}`,
		{ timeout: 10000 },
		async () => {
			...
			try {
				...
				const modelStream = (ordinal) => {
					const item =
						ordinal === 1
							? patchCall
							: message("fixture applied", "final_answer")
					const response = {
				...
				const handler = createOpenAIOAuthFetchHandler({
					...
					fetch: async (url, init) => {
						...
						if (requests === 2) {
							...
							assert.deepEqual(JSON.parse(replay.arguments), {
								operation: patch,
							})
							const result = body.input.find(
								(item) =>
				...
				const client = new OpenAI({
					...
					fetch: async (url, init) => {
						...
						const nativeResult = sdkBody.input?.find(
							(item) => item.type === "apply_patch_call_output",
						)
						if (nativeResult) {
							assert.equal(nativeResult.call_id, patchCall.call_id)
							assert.equal(nativeResult.status, expectedStatus)
							assert.equal(
								Object.hasOwn(nativeResult, "output"),
								expectedOutput !== "",
							)
						}
						const headers = new Headers(init.headers)
